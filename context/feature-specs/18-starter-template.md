...
Add a small starter template library so users can start a canvas from a
pre-built diagram instead of building from scratch.
...
1. Create `components/editor/starter-templates.ts`.
   ...
   - at least three templates, such as microservices, CI/CD pipeline, and
     event-driven system
   ...
   Use the shared canvas types and existing node color palette. Add small helper
   functions if needed to keep the template data readable.
...
4. Wire starter templates into the editor.
   ...
   - when a template is selected, clear all existing nodes and edges first in
     the canvas
   - add the selected template nodes and edges after the canvas is cleared
   - make sure the starter template replaces the current canvas instead of being
     added on top of existing work
...
- Import action replaces the current canvas through the existing node and edge
  state flow.
