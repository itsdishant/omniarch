...
- Complete (`21-canvas-autosave` — Vercel Blob canvas persistence and save status)
...
- Next feature unit: `context/feature-specs/22-design-agent-api.md`
...
- **Liveblocks + React Flow integration**: Hooks and providers come from `@liveblocks/react/suspense`. `liveblocks.config.ts` types Presence (`cursor`, `thinking`), Storage (`flow` LiveObject of node/edge LiveMaps), UserMeta, RoomInfo, and related slots. Canvas uses `LiveblocksProvider` (`throttle={16}`, `preventUnsavedChanges`, badge `top-right`) + `RoomProvider`. Loading uses `ClientSideSuspense`; errors use `react-error-boundary`. `useLiveblocksFlow({ suspense: true })` syncs nodes/edges. `@liveblocks/react-flow` `Cursors` writes pointer position into Presence and renders other users from session `userInfo` (not `resolveUsers`). Canvas-only participant avatars read the same `useOthers()` list, exclude the Clerk `userId`, and keep the Clerk `UserButton` separate. Connection issues use `useErrorListener` and `useLostConnectionListener`. Custom node type `canvasNode` is registered via `nodeTypes`. Shape visuals live in `canvas-shapes.tsx`; node chrome (handles, resize, color toolbar, labels) lives in `canvas-node.tsx`; `canvas-wrapper.tsx` owns providers, Liveblocks flow, drop, presence avatars, cursors, and overlay controls.
