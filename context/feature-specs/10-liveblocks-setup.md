...
Denied access must return JSON `{ "error": "forbidden", "reason": "..." }` so the Liveblocks client does not retry forever. Use `403` for missing project membership.

Permissions stay in Prisma (owner / collaborator). Do not switch this route to ID tokens unless room `usersAccesses` is synced on invite/remove.
...
- auth route verifies project access and returns `{ error: "forbidden" }` on deny
