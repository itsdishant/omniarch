...
Add shared AI activity indicators so everyone in the room can see when
generation is in progress. This unit is only for UI, presence, and realtime
status signals. Do not add the actual AI generation flow yet.
...
2. Add a shared AI status feed.
   - check the existing Liveblocks setup and installed agent-related features
     first
...
4. Add thinking indicators to live cursors.
   - when a participant has `thinking: true` in presence, show a small spinner
     in their cursor name badge
