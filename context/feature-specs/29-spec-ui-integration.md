...
and download specs from the existing ai sidebar specs tab.

## Implementation

1. Spec list
   - in the right sidebar (Specs tab), show a list of specs for the current
     project
...
1. Preview modal
   ...
   - fetch the spec content through an existing endpoint (do not access Blob
...
- assume ProjectSpec only provides metadata, content must be fetched
  separately
