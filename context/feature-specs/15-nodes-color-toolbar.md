...
Add a small floating color toolbar so selected nodes can change both their
background and text color directly on the canvas.
...
1. Check `ui-context.md` for the node color palette.
   ...
   Reuse existing theme colors if they already exist in the `global.css`.
   Otherwise, keep the palette in the canvas types/constants, such as
   `types/canvas.ts`.
...
4. Selected nodes should visually reflect their active color pair.
   ...
   The node background updates to the selected color, and the text automatically
   updates to its paired text color.
