...
- `05-prisma` — `Project` / `ProjectCollaborator` in `prisma/models/project.prisma`, cached `lib/prisma.ts` (Accelerate vs `@prisma/adapter-pg`), migration `project_models`
...
- Implemented `context/feature-specs/05-prisma.md`. Starter User/Post tables dropped in `project_models`. `npm run build` passes.
