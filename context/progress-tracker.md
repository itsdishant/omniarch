...
- None. Prisma project models unit is done.
...
- Clerk is the auth layer. `proxy.ts` runs `clerkMiddleware()` only. Protected pages and layouts call `auth.protect()`. Sign-in and sign-up stay public. Clerk appearance uses the `dark` theme with CSS variable overrides only (no hardcoded colors). The editor workspace lives at `/editor`; `/` only redirects signed-in users there.
